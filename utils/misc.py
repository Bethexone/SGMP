...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], split=True):
    ...
    return NestedTensorMask(tensor, mask)


class NestedTensorMask(object):
    def __init__(self, tensors, mask: Optional[Tensor]):
        self.tensors = tensors
        self.mask = mask

    def to(self, device):
        # type: (Device) -> NestedTensorMask # noqa
        ...
        if mask is not None:
            ...
            cast_mask = None
        return NestedTensorMask(cast_tensor, cast_mask)
...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        logging.debug('Not using distributed mode')
    ...
    logging.debug('| distributed init (rank {}): {}'.format(
