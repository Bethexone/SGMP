...
def resize(clip, target, size, max_size=None, least_factor=None):
    # size can be min_size (scalar) or (w, h) tuple

    def get_size_with_aspect_ratio(image_size, size, max_size=None, least_factor=None):
        w, h = image_size
        ...
        if least_factor is not None:
            # 将宽高调整为least_factor的倍数（四舍五入）
            ow = round(ow / least_factor) * least_factor
            oh = round(oh / least_factor) * least_factor
    ...
    def get_size(image_size, size, max_size=None):
        if isinstance(size, (list, tuple)):
            ...
            return get_size_with_aspect_ratio(image_size, size, max_size, least_factor)
...
class RandomResize(object):
    def __init__(self, sizes, max_size=None, least_factor=None):
        assert isinstance(sizes, (list, tuple))
        self.sizes = sizes
        self.max_size = max_size
        self.least_factor = least_factor

    def __call__(self, img, target=None):
        size = random.choice(self.sizes)
        return resize(img, target, size, self.max_size, self.least_factor)
...
class InverseNormalizeTransforms:
    ...
    def __call__(self, data):
        # import ipdb; ipdb.set_trace()
        tr, _ = self.inverse_normalize(data, None)
